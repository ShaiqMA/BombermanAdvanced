...
public class Shader
{
  ...
  {
    // early out if uniform does not exist, which happens when it is optimized out
  ...
  public void SetInt2(string name, Vector2i data)
  {
    // early out if uniform does not exist, which happens when it is optimized out
    if (!_uniformLocations.ContainsKey(name)) return;
  ...
  public void SetFloat2_Internal(string name, Vector2 data)
  {
    GL.UseProgram(Handle);
